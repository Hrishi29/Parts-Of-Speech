...
//Admin Validation 
if(isset($_POST['AdminTxt']) and isset($_POST['AdminPass'])) {
    $Adminname = mysqli_real_escape_string($conn,test_input($_POST['AdminTxt']));
    $AdminPassword = mysqli_real_escape_string($conn,test_input($_POST['AdminPass']));
    $query_admin = "SELECT Email, Password FROM admin where Email='".$Adminname."' AND Password='".$AdminPassword."'";
    $result = $conn->query($query_admin);
    if($result){
...
//Table Data for Admin Table from users table
...
// load default set    
if(isset($_POST['currSets']) and isset($_POST['UpdateAdmin'])){
   if(mysqli_real_escape_string($conn,test_input($_POST['currSets'])) == 'set'){
        $adminValid = mysqli_real_escape_string($conn,test_input($_POST['UpdateAdmin']));
        $query_sets = "SELECT QuestionSet, StartDate, EndDate FROM adminquestionset where Current='live' AND Username='".$adminValid."'";
...
//Dynamic select
if(isset($_POST['modifySets']) and isset($_POST['UpdateAdmin'])){
    $dynset = mysqli_real_escape_string($conn,test_input($_POST['modifySets']));
    $adminValid = mysqli_real_escape_string($conn,test_input($_POST['UpdateAdmin']));
    $query_sets = "SELECT StartDate, EndDate FROM adminquestionset where QuestionSet='".$dynset."' AND Username='".$adminValid."'";
    $query_result = $conn->query($query_sets);
    if($query_result){
        $row = $query_result->fetch_assoc(); 
        $myJSON = json_encode($row);
...
if(isset($_POST['UpdateStart']) and isset($_POST['UpdateEnd']) and isset($_POST['UpdateLive']) and isset($_POST['UpdateAdmin'])){
    ...
    if(empty($startEntry) || empty($endEntry)){ 
        $setNull = "UPDATE adminquestionset SET StartDate = NULL, EndDate = NULL where Username='".$adminValid."' AND QuestionSet='".$liveEntry."'";
        ...
        if($resulterase){
            $set_date = "UPDATE adminquestionset SET StartDate = '".$startEntry."', EndDate = '".$endEntry."', Current = 'live' where Username='".$adminValid."' AND QuestionSet='".$liveEntry."'";
...
// Update users table from admin portal
